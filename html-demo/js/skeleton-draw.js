...
const SkeletonDraw = (() => {
  ...
  // ── 配置 ─────────────────────────────────────────────────────────
  let config = {
    lineColor: null,       // null = 分部位彩色；否则用统一色
    lineWidth: 3,
    dotColor: '#ffffff',
    dotRadius: 5,
    showLabels: true,
    upperOnly: false,
    minVisibility: 0.3,    // 低于此置信度的关键点不渲染
  };
  ...
  function draw(ctx, landmarks, canvasW, canvasH) {
    ...
    const connections = config.upperOnly ? CONNECTIONS_UPPER : CONNECTIONS_FULL;
    ...
    landmarks.forEach((lm, idx) => {
      ...
      const r = isKeyJoint ? config.dotRadius + 2 : config.dotRadius - 1;
      const dotColor = config.lineColor || (isKeyJoint ? '#ffffff' : 'rgba(255,255,255,0.6)');
