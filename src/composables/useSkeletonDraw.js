...
export function useSkeletonDraw() {
  const setConfig = (opts) => {
    Object.assign(config.value, opts)
  }
  ...
  const draw = (ctx, landmarks, canvasW, canvasH) => {
    ...
    connections.forEach(([si, ei]) => {
      ...
      if (!s || !e) return
      if (s.visibility < config.value.minVisibility || e.visibility < config.value.minVisibility) return
  ...
  const drawLabels = (ctx, landmarks, w, h) => {
    const labelMap = {
  ...
  const getJointAngles = (landmarks) => {
    if (!landmarks) return {}
